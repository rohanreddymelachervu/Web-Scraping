...
topN = (int)(input("Enter topN results "))
query = QueryParser("content", ix.schema).parse(query_str)
results = ix.searcher(weighting=scoring.Frequency).search(query,limit=topN)
for i in range(topN):
    print(results[i]['title'], str(results[i].score), results[i]['textdata']); print("\n")
